...
def get_sp500_data(start=dt.datetime.strptime("1997-01-01", "%Y-%m-%d"),
    ...
    names = list(symbols_table.loc[:, "Security"])

    dict(zip(symbols, names))

    sp500 = None
    for i in range(len(symbols)):
        sleep(sleeptime)
        if verbose:
            print("Processing: " + symbols[i] + "...", end='')
        try:
            if use_quandl:
                s_data = quandl.get("WIKI/" + symbols[i], start_date=start, end_date=end, authtoken='ACdPaLLDhak82Uf9au9C')
                if adjust:
                    s_data = s_data.loc[:, "Adj. Close"]
                else:
                    s_data = s_data.loc[:, "Close"]
            else:
                s_data = web.DataReader(symbols[i], "google", start, end).loc[:, "Close"]
            s_data.name = symbols[i] + ',' + names[i]
            s_data.dropna()
            if s_data.shape[0] > 1:
                if sp500 is None:
    ...
    badsymbols = list(set(symbols[i]) - set(sp500.columns))
...
# This is a playground file, don't use it!!!